...
{
    uint8_t irq = 0, payload_available = 0;
    NRF24_Get_Status(&irq);

    payload_available = NRF24_Unload_payload(nrf_temp_rx_ram);
    ...
    nrf_receive_raw.roll = (int16_t) (GY_Roll() * 100.0f);
    nrf_receive_raw.pitch = (int16_t) (GY_Pitch() * 100.0f);
    nrf_receive_raw.yaw = (int16_t) (GY_Yaw() * 100.0f);
    nrf_receive_raw.temp = (int16_t) (Fusion_Temp() * 100.0f + 0.5f);
    nrf_receive_raw.height = MS_Height();
    nrf_receive_raw.acc_L1 = Drone_Motor.value_L1;
    nrf_receive_raw.acc_L2 = Drone_Motor.value_L2;
    nrf_receive_raw.acc_R1 = Drone_Motor.value_R1;
    nrf_receive_raw.acc_R2 = Drone_Motor.value_R2;
    ...
    if(irq == 96 && payload_available == 0)
    {
        data_exchange_lock_receive = 1;

        if(nrf_temp_rx_ram[0] == PACK_Type_Control)
        {
            package_is_control_message = 1;
            memcpy(&nrf_send_raw, nrf_temp_rx_ram, 32);

            //printf("Received controlling pack.\n");
        }
        else if(nrf_temp_rx_ram[0] == PACK_Type_Setting)
        {
            package_is_control_message = 0;

            package_new_setting = 1;

            memcpy(&nrf_manual_raw, nrf_temp_rx_ram, 32);

            if(nrf_manual_raw.pid_type == NRF_PID_Roll)
            {
                flash_pid_float.kp_roll = nrf_manual_raw.temp_kp;
                flash_pid_float.ki_roll = nrf_manual_raw.temp_ki;
                flash_pid_float.kd_roll = nrf_manual_raw.temp_kd;
            ...
            else if(nrf_manual_raw.pid_type == NRF_PID_Pitch)
            {
                flash_pid_float.kp_pitch = nrf_manual_raw.temp_kp;
                flash_pid_float.ki_pitch = nrf_manual_raw.temp_ki;
                flash_pid_float.kd_pitch = nrf_manual_raw.temp_kd;

                printf("pitch pid get. \n");
            ...
            {
                flash_pid_float.kp_yaw = nrf_manual_raw.temp_kp;
                flash_pid_float.ki_yaw = nrf_manual_raw.temp_ki;
                flash_pid_float.kd_yaw = nrf_manual_raw.temp_kd;
