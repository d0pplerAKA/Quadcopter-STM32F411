...
{        
    NRF24_Get_Status(&nrf_status);

    NRF24_Unload_payload(nrf_temp_rx_ram);
    
    if(nrf_status == 96)
        memcpy(&nrf_receive_raw, nrf_temp_rx_ram, 32);
...
/*
* Get NRF dynamic payload length
*/
void NRF24_Get_Dynamic_payload_length(uint8_t *payload_len)
{
    spi_read_byte(NRF_R_REGISTER | NRF_DYNPD, payload_len, NRF_NOP);
}

/*
* Set NRF dynamic payload length
*/
void NRF24_Set_Dynamic_payload_length(uint8_t pipe_num, uint8_t state)
{
    uint8_t rf_payload, temp_rf_payload = 0x00;

    NRF24_Get_Dynamic_payload_length(&rf_payload);
    temp_rf_payload = rf_payload;

    nrf24_set_reg_bit(&temp_rf_payload, pipe_num, state);

    spi_write_byte(NRF_W_REGISTER | NRF_DYNPD, temp_rf_payload);
}
...
/*
* Set special NRF features
* Dynamic payload
*/
void NRF24_Cmd_dynamic_payload(uint8_t state)
{
    uint8_t rf_feature;
    NRF24_Get_Feature(&rf_feature);

    nrf24_set_reg_bit(&rf_feature, 2, state);
    spi_write_byte(NRF_W_REGISTER | NRF_FEATURE, rf_feature);
}
...
void NRF24_Cmd_payload_ack(uint8_t state)
{
    ...
    nrf24_set_reg_bit(&rf_feature, 1, state);
    spi_write_byte(NRF_W_REGISTER | NRF_FEATURE, rf_feature);
}

/*
* Set special NRF features
* TX Payload without Ack
*/
void NRF24_Cmd_tx_payload_no_ack(uint8_t state)
{
    ...
    nrf24_set_reg_bit(&rf_feature, 0, state);
    spi_write_byte(NRF_W_REGISTER | NRF_FEATURE, rf_feature);
}
...
{
    uint8_t rf_payload;
    spi_read_byte(NRF_R_RX_PL_WID, &rf_payload, NRF_NOP);
...
{   
    ...
    spi_write_bytes((NRF_W_ACK_PAYLOAD + pipe_num), data, 32);
}
